...
// The set of operating systems that can be used.
var (
	Linux   = newOS("linux")
	Darwin  = newOS("darwin")
...
// OS represents an operating system option.
type OS struct {
	value string
}
...
// ParseOS parses the string value and returns an OS if one exists.
...
// MustParseOS parses the string value and returns an OS if one exists. If
