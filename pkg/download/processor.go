...
// The set of processors that can be used. ROCm is included for AMD GPU
// targets — leejet/stable-diffusion.cpp ships both Windows and Linux ROCm
// zips. CUDA is supported on Windows only at the upstream level; Linux/CUDA
// has no upstream artifact (a future malina-builder will fill that gap).
var (
...
// Processor represents a hardware backend option.
...
// MustParseProcessor parses the string value and returns a Processor if one
