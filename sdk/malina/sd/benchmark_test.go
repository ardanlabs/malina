// Benchmarks exercise the low-level binding against supported model families.
package sd
...
func BenchmarkGenerateImageFlux2(b *testing.B) {
	...
	cparams := benchContextParams()
	cparams.DiffusionModelPath = manifest.Files[string(models.RoleDiffusion)]
	cparams.VAEPath = manifest.Files[string(models.RoleVAE)]
	cparams.LLMPath = manifest.Files[string(models.RoleLLM)]
