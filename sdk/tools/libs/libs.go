// Package libs installs and inspects stable-diffusion.cpp library bundles.
package libs
...
// against. `malina libs pull` uses this when no -v flag is supplied so first
...
// re-running the FFI sizeof tests in sdk/malina/sd.
...
func Get(architecture, osName, processor, version, dest string) error {
	return GetWithProgress(architecture, osName, processor, version, dest, downloader.DefaultProgressTracker())
}
